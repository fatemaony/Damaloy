  ...
  return (
    <div className="relative w-full h-[400px] md:h-[500px] overflow-hidden group">
      <AnimatePresence mode='wait'>
        ...
        >
          <div className="container mx-auto px-6 md:px-12 flex flex-col-reverse md:flex-row items-center justify-between h-full gap-8">
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="text-green-600 font-bold tracking-wider text-sm uppercase bg-white/50 px-3 py-1 rounded-full">
                  {banners[current].subtitle}
                </span>
              </div>

              <h2 className="text-4xl md:text-6xl font-bold text-gray-800 mb-4 font-aladin leading-tight">
            ...
            <motion.div
              ...
              animate={{ x: 0, opacity: 1 }}
              transition={{ delay: 0.2, duration: 0.5 }}
              className="w-full md:w-1/2 h-1/2 md:h-full flex items-center justify-center relative"
            >
              <img
                src={banners[current].image}
                alt={banners[current].title}
                className="w-auto h-full max-h-[300px] md:max-h-[450px] object-contain drop-shadow-2xl"
              />
            </motion.div>
          </div>
        </motion.div>
      ...
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-3 rounded-full shadow-md transition-all opacity-0 group-hover:opacity-100 z-20"
      >
      ...
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-3 rounded-full shadow-md transition-all opacity-0 group-hover:opacity-100 z-20"
      >
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
        {banners.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all ${current === index ? "bg-primary w-8" : "bg-gray-400/50 hover:bg-white"
              }`}
          />
        ))}
      </div>
