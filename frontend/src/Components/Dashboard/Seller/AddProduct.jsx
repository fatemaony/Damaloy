...
import { Store, Package, DollarSign, Ruler, Image as ImageIcon, FileText, Save, Loader2 } from 'lucide-react';
...
const AddProduct = () => {
  ...
  useEffect(() => {
    const fetchSellerInfo = async () => {
      ...
      try {
        console.log("Fetching seller info for:", user.email);
        const response = await axiosInstance.get(`/api/sellers/email/${user.email}`);
        console.log("Seller info response:", response.data);
        if (response.data.success) {
          setSellerInfo(response.data.data);
        }
        console.log("Seller info:", response.data.success);
      } catch (error) {
    ...
    try {
      if (!sellerInfo) {
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Seller information not found. Please try again later.',
        });
        return;
      }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary focus:border-transp arent transition-all outline-none"
