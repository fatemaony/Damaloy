...
const MyProducts = () => {
  ...
  const fetchMyProducts = async () => {
    ...
    setLoading(true);
    try {
      // First get seller ID using email
  ...
  const handleUpdate = (id) => {
    navigate(`/dashboard/seller/updateProduct/${id}`);
  };
  ...
  if (error) {
    return (
      <div className="text-center text-red-500 py-10">
        <p>{error}</p>
        <button onClick={fetchMyProducts} className="mt-4 btn btn-primary btn-sm">Retry</button>
      </div>
    );
  }
